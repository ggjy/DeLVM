...
from internlm.data.batch_sampler import get_dpsampler_dataloader, StaticBatchSampler
...
from internlm.data.packed_dataset import (get_packed_dataset_without_short_length, PackedDataset,
...
from internlm.utils.model_checkpoint import get_current_device
...
@llm_timeout(func_name="initialize_teacher")
def initialize_teacher():
    ...
    model = MODEL_INITIALIZER.get_module(module_name=gpc.config.teacher_type)(**(gpc.config.teacher))
        ...
        model = nn.ModuleList(
            [
                NaiveAMPModel(
                    ...
                    dtype=gpc.config.teacher.get("dtype", torch.half),
                    sync_buffer=False,
                )
                for _m in model
            ]
    ...
    folder = gpc.config.teacher_ckpt_folder
    if gpc.is_rank_for_log():
        logger.info(f"loading teacher model from: {folder}")
    ...
    if not os.path.exists(folder):
        raise RuntimeError(f"teacher ckpt at '{folder}' not found!")
    else:
        fns = os.listdir(folder)

    max_pp, max_tp = 0, 0
    for fn in fns:
        if fn.startswith("model_t") and not fn.endswith(".md5") and not fn.endswith(".upload_record"):
    ...
    with open(fp, "rb") as f:
        states = torch.load(f, map_location=get_current_device())
    ...
    if gpc.is_rank_for_log():
        logger.info(f"loading teacher model successfully")
...
def load_new_batch(train_dl: DataLoader, train_iter: Iterable, train_state: TrainState):
    ...
    try:
        ...
        if gpc.is_rank_for_log():
            logger.warning('Reach the end of dataset! Epoch %s Length is %s', gpc.epoch, len(train_dl.dataset))
            gpc.epoch += 1
...
@llm_timeout(func_name="load_new_batch_stop")
def load_new_batch_stop(train_dl: DataLoader, train_iter: Iterable, train_state: TrainState):
    """
    Load and return the new batch data based on training data loader. Return None when StopIteration
