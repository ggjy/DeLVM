...
class CheckpointManager:
    ...
    def __init__(
        ...
        # model_hf to internal representation
...
now step_count is {train_state.step_count}",
    ...
    def is_now_to_save_ckpt(self, train_state, data_iter_stop=False) -> (bool, CheckpointSaveType, bool):
        if data_iter_stop:
            return True, CheckpointSaveType.NORMAL_CHECKPOINT, True
    ...
    def try_save_checkpoint(self, train_state, data_iter_stop=False):
        if not self.enable_save_ckpt:
            return False

        save_ckpts, save_type, now_break = self.is_now_to_save_ckpt(train_state, data_iter_stop)
